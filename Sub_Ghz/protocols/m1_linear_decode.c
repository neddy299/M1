...
*  m1_linear_decode.c
*
*  M1 sub-ghz Linear 10-bit decoding (Linear Delta-3 / MegaCode)
*  Encoding: short-high + long-low → bit 0; long-high + short-low → bit 1
*  te_short ~500μs, te_long ~1500μs (1:3 ratio)
*  10-bit DIP switch code at 300 or 310 MHz
*/
...
#define M1_LOGDB_TAG	"SUBGHZ_LINEAR"
...
uint8_t subghz_decode_linear(uint16_t p, uint16_t pulsecount)
{
    ...
    /* Linear: tri-state encoding with short/long pulse pairs
     * Each bit is a pair: short+long=0, long+short=1 */
