...
*  m1_hormann_decode.c
*
*  M1 sub-ghz Hörmann BiSecur 44-bit decoding
*  Encoding: short-high + long-low → bit 0; long-high + short-low → bit 1
*  te_short ~500μs, te_long ~1000μs (1:2 ratio)
*  Garage door openers on 433.92/868.35 MHz (EU market)
*
*  Packet: 44 bits
*    bits 0-3:   button ID
*    bits 4-27:  serial number (24-bit)
*    bits 28-43: counter/status
*/
...
#define M1_LOGDB_TAG	"SUBGHZ_HORMANN"
...
uint8_t subghz_decode_hormann(uint16_t p, uint16_t pulsecount)
{
    ...
    if (bits_count >= max_bits)
    {
        ...
        /* Extract fields */
        subghz_decenc_ctl.n8_buttonid = (uint8_t)((code >> 40) & 0x0F);
        subghz_decenc_ctl.n32_serialnumber = (uint32_t)((code >> 16) & 0x00FFFFFF);
